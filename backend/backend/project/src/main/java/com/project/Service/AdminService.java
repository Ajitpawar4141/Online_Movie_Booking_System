...
public class AdminService {
	...
	public String saveTheater(AddTheater nt) {
				
		User manager = userdao.getById(nt.getManagerid());
		Theaters th=new Theaters(nt.getName(),nt.getAddress(),nt.getCity(),nt.getScreens_count(),nt.getFacilities(),nt.getCountersCount(),nt.getGstNumber(),manager);
		homedao.save(th);
		return "saves";
	}
	...
	public Theaters updateTheatre(int theatreId, AddTheater updateTheatre) {
		...
		t.setName(updateTheatre.getName());
		...
		t.setScreens_count(updateTheatre.getScreens_count());
